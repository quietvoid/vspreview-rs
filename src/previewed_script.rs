...
impl PreviewedScript {
    pub fn new(script_path: PathBuf) -> Self {
        ...
        Self {
            env: RwLock::new(Environment::from_file(&script_file, EvalFlags::SetWorkingDir).unwrap()),
    ...
    pub fn reload(&self) {
        ...
        env.eval_file(&self.script_file, EvalFlags::SetWorkingDir).unwrap();
    }
    ...
    pub fn get_frame(&self, frame_no: u32) -> image::ImageBuffer<image::Rgba<u8>, Vec<u8>> {
        ...
        let resize_plugin = env.get_core().unwrap().get_plugin_by_id("com.vapoursynth.resize").unwrap().unwrap();
        ...
        let (r, g, b): (&[u8], &[u8], &[u8]) = (frame.plane(0).unwrap(), frame.plane(1).unwrap(), frame.plane(2).unwrap());
    ...
    fn to_rgba_buf(&self, res: Resolution, r: &[u8], g: &[u8], b: &[u8]) -> image::ImageBuffer<image::Rgba<u8>, Vec<u8>> {
